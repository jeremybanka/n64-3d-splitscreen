...
export def build-rom [config: record] {
    ...
    let flags = [
        ...
        $'-DPROFILE=($config.profile)' $'-DBENCHMARK=($config.benchmark)'
    ...
    let identity = {implementation: $implementation, elf: (digest $elf), strip: (digest (tool strip)), tools: $packaging}
    if not (same-state build/rom.json $identity $rom) {
        ...
        command ($install | path join bin n64tool) [--title 'BUNNY MEADOW' --toc --output $temporary --align 256 $'($elf).stripped' --align 8 $'($elf).sym' --align 8]
